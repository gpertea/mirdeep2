...
## options
my %options=();
getopts("p:m:r:B:s:t:y:dokuc:nxg:e:f:vjT:PWU",\%options);
...
\t-B           directory path with miRNA_mature and miRNA_precursor bowtie indexes
...
if(not $options{'r'} or not $options{'B'}) {
    die $usage;
}
my $bwdir=$options{'B'};
...
my ( $name0, $path0, $extension0 ) = fileparse ( "$bwdir/precursor.converted", '\..*' );

#my ( $name1, $path1, $extension1 );
#( $name1, $path1, $extension1 ) = fileparse ( $options{'m'}, '\..*' );# if(not defined $options{'w'});
#( $name1, $path1, $extension1 ) = fileparse ( "$bwdir/mature.converted", '\..*' );
my ($mature_fa, $mature_bwt, $mature_fbase)=( "$bwdir/mature.converted", "$bwdir/mature2precursor.bwt", 'mature_hsa.fa' );
...
if(0){
    if(not $options{'c'}){
        open IN,"<$options{'r'}" || die("Error opening reads file\n");
        while(<IN>){
            next if($_ !~ /^>/);
            if(/^>(\S\S\S)_\S+_x(\d+)$/){
                $samples{$1} += $2;
        ...
        if(-f $options{'c'}){
            open IN,"<$options{'c'}" || die("file -c not found!\n");
            while(<IN>){
                chomp;
                if(/^(\S+)\s+(\S+)$/){
...
#     $command = "make_html2.pl -q $outdir/miRBase.mrd -k $matcvt -t $organisms{$species} -y $time $opt_d $opt_o -i $outdir/mature_mapped.arf $starf $opt_l $opt_m -M miRNAs_expressed_all_samples_$time.csv $opt_P $opt_W";
...
# }elsif($rorganisms{$species}){
    ...
    $command="make_html2.pl -q $outdir/miRBase.mrd -k $mature_fa -t $species -y $time $opt_d $opt_o -i $outdir/${mature_fbase}_mapped.arf $starf $opt_l $opt_m -M miRNAs_expressed_all_samples_$time.csv $opt_P $opt_W";
...
## map mature sequences against precursors
 #  print STDERR "mapping mature miRNAs against precursors\n";
    #if(!$options{'w'}){
    #    $cmd="bowtie -p $threads -f -v 0 -a --best --strata --norc $bwdir/miRNA_precursor".
    #    " mature.converted ${mature_fbase}_mapped.bwt 2>bowtie_mature.out";
    #    print STDERR "run: $cmd\n";
    #    $err = `$cmd`;
    #}

## map reads against precursors
    print STDERR "mapping read sequences vs mature miRNAs\n";
    $bwf="${name2}_mapped2mature.bwt";
    $cmd="bowtie -p $threads -f -v 0 -a --best --strata --norc $bwdir/miRNA_mature ".
    " $name2.converted  $bwf 2>bowtie_reads_vs_mature.errout";
    print STDERR "run: $cmd\n";
    $err=`$cmd`;
    read_stats("$name2.converted",$bwf);

    print STDERR "mapping read sequences vs precursors\n";
    $bwf="${name2}_mapped.bwt";
    $cmd="bowtie -p $threads -f -v $mismatches -a --best --strata --norc $bwdir/miRNA_precursor ".
    " $name2.converted $bwf 2>bowtie_reads_vs_precursors.errout";
    $err=`$cmd`;
    print STDERR "run: $cmd\n";
    read_stats("$name2.converted", $bwf);
    if($options{'s'}) {
...
sub read_stats{
    ...
    print STDERR " Mapping statistics $f1 vs $f2:\n";
    ...
    print STDERR "#desc\ttotal\tmapped\tunmapped\t%mapped\t%unmapped\n";
...
sub ReadinPrecursorFile{
    my $id='';
    my $fc=$bwdir.'/precursor.converted';
...
sub ReadinMatureMappingFile{
    ...
    #my $mf="${name1}_mapped.bwt";
    my $mf=$mature_bwt;
    open IN,$mf or die "Mature mapping file $mf not found \n";
    my $cx = 0;
    my $id1 ='';
    my $id2='';
...
sub CreateOutputMRD{
    my %exprs;
    my @ex;
    open HPNE,">$outdir/precursor_not_expressed.csv" or die "FILE $outdir/precursor_not_expressed.csv could not be created\n";
    ...
    ## get mature mappings
    my ($mature,$star,$reads, $cmd);
    unless (-f "${mature_fbase}_mapped.arf") {
      $cmd="convert_bowtie_output.pl $mature_bwt > ${mature_fbase}_mapped.arf";
    ...
    my $id1= '';
    my $id2='';
    open IN,"<${mature_fbase}_mapped.arf" || die("Error opening ${mature_fbase}_mapped.arf\n");
    while(<IN>){
        $id1= '';
        $id2='';
        @line = split(/\t/);
    ...
    print "Mapped READS readin - DONE \n\n";
    ...
    for my $k1(sort keys %hash){
        ...
        #    next if($k1 !~ /$species/);
        #}
        ## skip hairpins with 0 expression for output.mrd file
        if(not $hash{$k1}{'r'}){
            print HPNE "$k1\n";
            next;
        }
